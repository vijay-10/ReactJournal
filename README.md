...
Which means we are not dividing software into small, well-defined modules, we use every services like, database, server or a UI of the application, in one Application file.
...
```
If we do not anyhting, i.e., pass an empty dependency array, then the useEffect runs everytime when the UI is rendered.
